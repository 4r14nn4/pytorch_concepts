...
class BnLearnDataset(ConceptDataset):
    ...
    def __init__(
            ...
            autoencoder_exclude: Optional[List[str]] = None,
    ):
        self.name = name
        self.seed = seed
        ...
        self.autoencoder_exclude = list(autoencoder_exclude or [])
    ...
    def build(self):
        ...
        concepts = df.copy()
        missing = [name for name in self.autoencoder_exclude if name not in df.columns]
        if missing:
            raise ValueError(f"Unknown autoencoder_exclude columns: {missing}")
        embedding_df = df.drop(columns=self.autoencoder_exclude)
...
class BnLearnDataModule(ConceptDataModule):
    """Wrap the Bayesian-network dataset with splitting and batch loading."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        dataset = BnLearnDataset(
            name=name, root=root, seed=generation_seed, n_gen=n_gen,
            concept_subset=concept_subset,
            label_descriptions=label_descriptions,
            autoencoder_kwargs=autoencoder_kwargs,
            autoencoder_exclude=autoencoder_exclude,
        )
        super().__init__(
            dataset=dataset, val_size=val_size, test_size=test_size,
            batch_size=batch_size, workers=workers, seed=seed, **kwargs,
        )
